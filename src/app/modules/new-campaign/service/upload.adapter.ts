...
export class DemoFilePickerAdapter extends FilePickerAdapter {
  ...
  public uploadFile(fileItem: FilePreviewModel): Observable<any> {
    ...
    form.append('icon', fileItem.file);
    const api =
      'https://demo.rewardadz.com/portal/campaigns/create/step2/icon/646f5bb5c7a8c187caadf07f';
    ...
    return this.http.request(req).pipe(
      map((res: HttpEvent<any>) => {
        if (res.type === HttpEventType.Response) {
          ...
          let total: any = 0;
          total = res.total;
          const uploadProgress = +Math.round((100 * res.loaded) / total);
          return {
